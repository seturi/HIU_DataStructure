...
Polynomial Polynomial::operator/(Polynomial& b)
{
   ...
   Polynomial remainder;      // 나눗셈 연산 중 생기는 나머지

   float cCoef;
   int cExp;

   for (int i = 0; i < terms; i++) {
      remainder.NewTerm(termArray[i].coef, termArray[i].exp);
   }

   if (termArray[0].exp < b.termArray[0].exp) {
      c = Polynomial();
      return c;
   }

   while (remainder.termArray[0].exp >= b.termArray[0].exp) {
      cCoef = remainder.termArray[0].coef / b.termArray[0].coef;
      cExp = remainder.termArray[0].exp - b.termArray[0].exp;

      c.NewTerm(cCoef, cExp);

      for (int i = 0; i < b.terms; i++) {
         b.termArray[i].coef *= -cCoef;
         b.termArray[i].exp += cExp;
      }
